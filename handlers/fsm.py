...
async def handle_user_text(event):
    ...
    auth_steps = {'reg_name', 'reg_unique', 'reg_password', 'auth_unique', 'auth_password'}
    profile_steps = {'wait_new_name', 'wait_new_unique', 'wait_new_password', 'wait_delete_login', 'wait_delete_password_final'}
...
async def handle_auth_step(event, step, text):
    if step == 'reg_name':
        ...
        token = get_token()
        try:
            reg_user(token=token, unique=data['unique'], first_name=data['first_name'], password=text, tg_id=event.sender_id)
