...
async def confirmdelete(event):
    ...
    await event.edit('⚠️ **ВИДАЛЕННЯ АКАУНТУ**\n\nДля підтвердження введіть ваш **Логін (unique)**:', buttons=remake_back)
    ...
    if step == 'wait_new_name':
        ...
        if res.status_code == 200:
            ...
            if 'No changes detected' in str(reason):
                await event.respond('⚠️ **Ви увели той самий пароль.**\nПридумайте нову комбінацію або натисніть «Назад».')
            else:
        ...
        user_state[event.sender_id] = {'step': 'wait_delete_password_final', 'delete_unique': text.strip()}
...
def _build_update_payload(user_data, unique=None, first_name=None, password=None):
    return {'unique': unique if unique is not None else user_data.get('unique', ''), 'first_name': first_name if first_name is not None else user_data.get('first_name', ''), 'middle_name': user_data.get('middle_name', ''), 'last_name': user_data.get('last_name', ''), 'password': password if password is not None else user_data.get('password', ''), 'phone': user_data.get('phone', 0), 'email': user_data.get('email', '')}
