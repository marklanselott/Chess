...
from api import delete_friend_from_api, get_token, get_user, get_user_by_unique, get_friends_list, send_friend_request, update_friend_request, get_friend_requests_for_me, get_friend_requests_my, cancel_friend_request
...
def register_friend_handlers(bot):
    ...
    bot.add_event_handler(handle_accept_request, events.CallbackQuery(pattern='acc_(.+)'))
    bot.add_event_handler(handle_reject_request, events.CallbackQuery(pattern='rej_(.+)'))
    ...
    bot.add_event_handler(handle_cancel_request, events.CallbackQuery(pattern='can_(.+)'))
    bot.add_event_handler(show_unfriend_menu, events.CallbackQuery(data='unfriend_list'))
    bot.add_event_handler(handle_unfriend_action, events.CallbackQuery(pattern='unf_id_(.+)'))
...
async def handle_requests_menu(event):
    ...
    buttons = [[Button.inline('📥 Вхідні (вам)', data='requests_in')], [Button.inline('📤 Вихідні (від вас)', data='requests_out')], [Button.inline('⬅️ Назад у меню', data='friend_main')]]
...
async def show_incoming(event):
    ...
    for req in reqs:
        ...
        text += f'\n👤 Від: **@{name}**'
        buttons.append([Button.inline('✅ Прийняти', data=f'acc_{r_id}'), Button.inline('❌ Відхилити', data=f'rej_{r_id}')])
...
async def handle_friend_step(event, step, text):
    ...
    if add_res.status_code == 200:
        ...
        if friend_tg_id:
            try:
                msg = f"🔔 **Нова заявка в друзі!**\n\nГравець **{my_info['searched'][0].get('first_name')}** (@{my_info['searched'][0].get('unique')}) хоче додати вас."
                req_buttons = [[Button.inline('✅ Прийняти', data=f"acc_{add_res.json().get('id')}"), Button.inline('❌ Відхилити', data=f"rej_{add_res.json().get('id')}")]]
        ...
        if 'already exists' in str(reason).lower():
            await event.respond('⚠️ **Заявку вже було надіслано раніше.**\nВона очікує підтвердження в розділі «Заявки» у вашого друга.', buttons=main_menu)
        else:
            await event.respond(f'❌ Помилка сервера: {(reason if reason else add_res.status_code)}', buttons=main_menu)
    else:
        await event.respond(f'❌ Не вдалося надіслати запит (Код: {add_res.status_code})', buttons=main_menu)
